...
### Fiber server
Fiber server general configuration.
...
```go
rkbootfiber.GetFiberEntry("greeter").SwEntry
```
...
```go
rkbootfiber.GetFiberEntry("greeter").CommonServiceEntry
```
...
```go
rkbootfiber.GetFiberEntry("greeter").TvEntry
```
...
```go
rkbootfiber.GetFiberEntry("greeter").StaticFileEntry
```
...
```go
rkbootfiber.GetFiberEntry("greeter").PromEntry
```

### Middleware/Interceptor

#### logging
Logging middleware will use log instance from **fiber.logger.zapLogger.ref** and **fiber.logger.eventLogger.ref**.
...
#### Metrics prometheus
**fiber.prom** must be enabled in order to use it.
